...
{                                                                                              \
	...
	(uiCount)++;                                                                               \
	vPort_SPI_writeDataNoWait((ucUnitNumber), (pcArr)[uiIndex]);	                               \
}
...
void vHOS_SPI_transceive(uint8_t ucUnitNumber, int8_t* pcOutArr, int8_t* pcInArr, uint32_t uiSize)
{
	...
	while(uiCount < uiSize)
	{
		...
		vPort_SPI_writeDataNoWait(ucUnitNumber, pcOutArr[uiIndex]);
		xSemaphoreTake(pxSPIHWMutexArr[ucUnitNumber], portMAX_DELAY);
		pcInArr[uiIndex] = ucPort_SPI_readDataNoWait(ucUnitNumber);
		uiCount++;
	}

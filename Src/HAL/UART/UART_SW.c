...
//{
//	volatile xHOS_UART_Unit_t* pxUnit = &pxUnitArr[ucUnitNumber];
//	volatile Rx_Transition_t xTrans;
//	volatile uint8_t uc1stPropertyAcheived, uc2ndPropertyAcheived;
//
//	volatile uint8_t ucNBits, ucBitCount = 0;
//	volatile uint64_t ulDeltaTimeUs;
//	volatile uint16_t usFrame = 0;
//
//	volatile TickType_t xCurrentTime = xTaskGetTickCount();
//	volatile TickType_t xEndTime = xCurrentTime + xTimeout;
//
...
//	if (usFrame >> (1+8+2+0) == 0 && ucBitCount >= (1+8+2+0))
...
//
//	/*
//	 * If edge time is less than time of one bit (de-bouncing noise), ignore
//	 * this edge.
//	 */
//	if (	xTrans.ulTimestamp - pxUnit->ulPrevEdgeTimestamp <
//			uiGET_BIT_TIME_US(ucUnitNumber)	)
//	{
//		return;
//	}
//
//	pxUnit->ulPrevEdgeTimestamp = xTrans.ulTimestamp;
//
//	/*	Get edge type	*/
//	xTrans.ucType = ucPORT_DIO_READ_PIN(	ucGET_RX_PORT(ucUnitNumber),
//											ucGET_RX_PIN (ucUnitNumber)	);
//
//	/*	If edge type is same as the previous edge, ignore new one	*/
//	if (xTrans.ucType == pxUnit->ucPrevEdgeType)
//	{
//		return;
//	}
//
//	pxUnit->ucPrevEdgeType = xTrans.ucType;
//
//	/*	Enqueue transition	*/
...
// */
//static volatile Rx_Transition_t mem[16];
...
//												(uint8_t*)mem,//pxUnit->pucBitQueueMemory,
//												&pxUnit->xBitQueueStatic	);
//
//		pxUnit->ulPrevDequeuedBitTimestamp = 0;
//
//		pxUnit->ulPrevEdgeTimestamp = 0;
//
